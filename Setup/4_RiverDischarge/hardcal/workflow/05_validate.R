...
period_valid <- c('2016-01-01', '2023-12-31')
...
gof_all_val <- list(gof_flow_val) %>%
  reduce(., left_join, by = 'run')

# Select the GOF values of interest
gof_sel_val <- select(gof_flow_val, run, nse_q, pb_q, kge_q)

# Print the mean GOF values for all runs
gof_sel_val[names(gof_sel)[names(gof_sel) != "run"]] %>% summarise_all(mean)

# For plotting options or extension to different variables, etc., see the the
# 04_analyze_results.R script.

gof_sel_plot <- gof_sel[gof_sel$run %in% sprintf("run_%03d", run_ids), ]

plot_cal_val(gof_sel_plot, gof_sel_val)
...
write_cal_file(par = par_vals,
               ...
               write_path = './',
               i_run = c(199, 200)) ## For single i_run = 1
